...
= \arg\max_{\theta} \, \log p(D | \theta) + \log \pi(\theta)
$$

| Estimator | Formula | Prior Used |
|------------|---------|-------------|
| **MLE** | $\arg\max_\theta \, p(D|\theta)$ | No prior |
| **MAP** | $\arg\max_\theta \, p(D|\theta)\pi(\theta)$ | Includes prior |
...
- If prior: $\theta \in \{0.2, 0.5, 0.7\}$ with  
  $\pi = \{0.1,0.01,0.89\}$

  → MAP = 0.7 (prior dominates)
...
If $X_i \sim \text{Bern}(\theta)$, $\pi(\theta) \sim \text{Beta}(\alpha,\beta)$:

- Posterior:
$$
\theta | X \sim \text{Beta}(\alpha + S_n,\;\beta + n - S_n)
$$
...
|---------|----------------------------------|----------------------------|
| Definition | Interval $C(D)$ s.t. $P_\theta(\theta \in C(D)) = 1-\alpha$ | $P(\theta \in I | D) = 1 - \alpha$ |
| Probability refers to | Data variability | Parameter uncertainty |
| Interpretation | In repeated sampling, captures true θ 95% of time | Given data, θ has 95% chance in interval |
