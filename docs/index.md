...
<!-- 파일명 타이틀 fallback 방지용: 시각적으로 숨긴 H1 -->
...
| Regularization control | Fine-grained tuning (depth/leaves/learning rate)                          |
| Early stopping         | Automatic overfitting control via validation split                        |
...
| `inference.py`                    | Self-inspection (confidence/abstention) + prediction interface       |
| `utils.py`                        | Shared helpers (I/O, validation, preprocessing)                      |
